...
///</summary>
public unsafe partial class ShadercCompileOptionsT : IUnmanagedWrapper<AdamantiumVulkan.Shaders.Interop.ShadercCompileOptions>
{
    ...
    public ShadercCompileOptionsT(in AdamantiumVulkan.Shaders.Interop.ShadercCompileOptions __Instance)
    {
        this.__Instance = __Instance;
    }

    public AdamantiumVulkan.Shaders.Interop.ShadercCompileOptions GetNativeValue() => __Instance;
    ///<summary>
    ...
    {
        int CalculateSize(string name, string value)
        {
            int totalSize = 0;
            if (!string.IsNullOrEmpty(name))
                totalSize += name.Length * sizeof(byte) + 1;
            if (!string.IsNullOrEmpty(value))
                totalSize += value.Length * sizeof(byte) + 1;
            return totalSize;
        }

        var totalSize = CalculateSize(name, value);
    ...
    {
        int CalculateSize(string reg, string set, string binding)
        {
            int totalSize = 0;
            if (!string.IsNullOrEmpty(reg))
                totalSize += reg.Length * sizeof(byte) + 1;
            if (!string.IsNullOrEmpty(set))
                totalSize += set.Length * sizeof(byte) + 1;
            if (!string.IsNullOrEmpty(binding))
                totalSize += binding.Length * sizeof(byte) + 1;
            return totalSize;
        }

        var totalSize = CalculateSize(reg, set, binding);
        byte[] rentedArray = null;
        var mainBuffer = totalSize <= QuantumBinding.Utils.MarshalingUtils.StackAllocThreshold ? stackalloc byte[totalSize] : (rentedArray = System.Buffers.ArrayPool<byte>.Shared.Rent(totalSize)).AsSpan(0, totalSize);
        try
        {
    ...
    {
        return new ShadercCompileOptionsT(in s);
    }
