// ----------------------------------------------------------------------------------------------
// <auto-generated>
// This file was autogenerated by QuantumBindingGenerator.
// Do not edit this file manually because you will lose all your changes after the next generation.
// </auto-generated>
// ----------------------------------------------------------------------------------------------

using System;
using System.Runtime.InteropServices;
using QuantumBinding.Utils;
using Adamantium.Vulkan.Core.Interop;

namespace Adamantium.Vulkan.Core;

public unsafe partial class StdVideoAV1TileInfo : IMarshallableObject, IMarshallable<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>
{
    public StdVideoAV1TileInfo()
    {
    }

    public StdVideoAV1TileInfo(in Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo native)
    {
        MarshalFrom(in native);
    }

    public StdVideoAV1TileInfoFlags Flags { get; set; }
    public byte TileCols { get; set; }
    public byte TileRows { get; set; }
    public ushort Context_update_tile_id { get; set; }
    public byte Tile_size_bytes_minus_1 { get; set; }
    public System.ReadOnlyMemory<byte> Reserved1 { get; set; }
    public System.ReadOnlyMemory<ushort> PMiColStarts { get; set; }
    public System.ReadOnlyMemory<ushort> PMiRowStarts { get; set; }
    public System.ReadOnlyMemory<ushort> PWidthInSbsMinus1 { get; set; }
    public System.ReadOnlyMemory<ushort> PHeightInSbsMinus1 { get; set; }


    public static implicit operator StdVideoAV1TileInfo(Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo s)
    {
        return new StdVideoAV1TileInfo(in s);
    }

    public int GetSize()
    {
        var size = QuantumBinding.Utils.SizeOfCache<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>.Size;
        if (!PMiColStarts.IsEmpty)
            size += PMiColStarts.Span.Length * QuantumBinding.Utils.SizeOfCache<System.UInt64>.Size;
        if (!PMiRowStarts.IsEmpty)
            size += PMiRowStarts.Span.Length * QuantumBinding.Utils.SizeOfCache<System.UInt64>.Size;
        if (!PWidthInSbsMinus1.IsEmpty)
            size += PWidthInSbsMinus1.Span.Length * QuantumBinding.Utils.SizeOfCache<System.UInt64>.Size;
        if (!PHeightInSbsMinus1.IsEmpty)
            size += PHeightInSbsMinus1.Span.Length * QuantumBinding.Utils.SizeOfCache<System.UInt64>.Size;
        return size;
    }

    public void MarshalTo(ref MarshallingContext<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo> context)
    {
        new StdVideoAV1TileInfoMarshaller(this, ref context);
    }

    public void MarshalFrom(in Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo native)
    {
        Flags = new StdVideoAV1TileInfoFlags(native.flags);
        TileCols = native.TileCols;
        TileRows = native.TileRows;
        Context_update_tile_id = native.context_update_tile_id;
        Tile_size_bytes_minus_1 = native.tile_size_bytes_minus_1;
        var tmpReserved1 = new byte[7];
        var reserved1p = native.reserved1[0];
        var pReserved1 = (byte*)System.Runtime.CompilerServices.Unsafe.AsPointer(ref System.Runtime.CompilerServices.Unsafe.AsRef(in reserved1p ));
        QuantumBinding.Utils.MarshalingUtils.MarshalFromPointerToArray(pReserved1, 7, tmpReserved1);
        Reserved1 = tmpReserved1;
        var arrayLengthPMiColStarts = native.TileCols;
        var tmpPMiColStarts = new ushort[arrayLengthPMiColStarts];
        QuantumBinding.Utils.MarshalingUtils.MarshalFromPointerToArray(native.pMiColStarts, arrayLengthPMiColStarts, tmpPMiColStarts);
        PMiColStarts = tmpPMiColStarts;
        var arrayLengthPMiRowStarts = native.TileRows;
        var tmpPMiRowStarts = new ushort[arrayLengthPMiRowStarts];
        QuantumBinding.Utils.MarshalingUtils.MarshalFromPointerToArray(native.pMiRowStarts, arrayLengthPMiRowStarts, tmpPMiRowStarts);
        PMiRowStarts = tmpPMiRowStarts;
        var arrayLengthPWidthInSbsMinus1 = native.TileCols;
        var tmpPWidthInSbsMinus1 = new ushort[arrayLengthPWidthInSbsMinus1];
        QuantumBinding.Utils.MarshalingUtils.MarshalFromPointerToArray(native.pWidthInSbsMinus1, arrayLengthPWidthInSbsMinus1, tmpPWidthInSbsMinus1);
        PWidthInSbsMinus1 = tmpPWidthInSbsMinus1;
        var arrayLengthPHeightInSbsMinus1 = native.TileRows;
        var tmpPHeightInSbsMinus1 = new ushort[arrayLengthPHeightInSbsMinus1];
        QuantumBinding.Utils.MarshalingUtils.MarshalFromPointerToArray(native.pHeightInSbsMinus1, arrayLengthPHeightInSbsMinus1, tmpPHeightInSbsMinus1);
        PHeightInSbsMinus1 = tmpPHeightInSbsMinus1;

    }
    public void* GetNativePointer<TContext>(ref TContext context) where TContext : IMarshallingContext, allows ref struct
    {
        var nativeSpan = context.AllocateNative<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>(1);
        var dataCursor = context.GetDataCursor();
        var internalContext = new MarshallingContext<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>(nativeSpan, dataCursor);
        this.MarshalTo(ref internalContext);
        context.SetDataCursor(internalContext.DataCursor);
        return System.Runtime.CompilerServices.Unsafe.AsPointer(ref nativeSpan[0]);
    }
    private ref struct StdVideoAV1TileInfoMarshaller
    {
        public StdVideoAV1TileInfoMarshaller(Adamantium.Vulkan.Core.StdVideoAV1TileInfo stdVideoAV1TileInfo, ref QuantumBinding.Utils.MarshallingContext<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo> context)
        {
            if (stdVideoAV1TileInfo.Flags != default)
            {
                fixed (Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfoFlags* pField = &context.Destination[0].flags)
                {
                    var fieldSpan = new System.Span<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfoFlags>(pField, 1);
                    var childContext = new MarshallingContext<Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfoFlags>(fieldSpan, context.DataCursor);
                    stdVideoAV1TileInfo.Flags.MarshalTo(ref childContext);
                    context.DataCursor = childContext.DataCursor;
                }
            }

            context.Destination[0].TileCols = stdVideoAV1TileInfo.TileCols;

            context.Destination[0].TileRows = stdVideoAV1TileInfo.TileRows;

            context.Destination[0].context_update_tile_id = stdVideoAV1TileInfo.Context_update_tile_id;

            context.Destination[0].tile_size_bytes_minus_1 = stdVideoAV1TileInfo.Tile_size_bytes_minus_1;

            ref var tmpDestination0 = ref context.Destination[0];
            fixed (byte* pDest = tmpDestination0.reserved1)
            {
                QuantumBinding.Utils.MarshalingUtils.MarshalFixedArrayToPointer(stdVideoAV1TileInfo.Reserved1.Span, pDest, 7);
            }

            if (!stdVideoAV1TileInfo.PMiColStarts.IsEmpty)
            {
                context.Destination[0].pMiColStarts = QuantumBinding.Utils.MarshalingUtils.MarshalBlittableArrayToPointer<ushort, Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>(stdVideoAV1TileInfo.PMiColStarts.Span, ref context);
            }

            if (!stdVideoAV1TileInfo.PMiRowStarts.IsEmpty)
            {
                context.Destination[0].pMiRowStarts = QuantumBinding.Utils.MarshalingUtils.MarshalBlittableArrayToPointer<ushort, Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>(stdVideoAV1TileInfo.PMiRowStarts.Span, ref context);
            }

            if (!stdVideoAV1TileInfo.PWidthInSbsMinus1.IsEmpty)
            {
                context.Destination[0].pWidthInSbsMinus1 = QuantumBinding.Utils.MarshalingUtils.MarshalBlittableArrayToPointer<ushort, Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>(stdVideoAV1TileInfo.PWidthInSbsMinus1.Span, ref context);
            }

            if (!stdVideoAV1TileInfo.PHeightInSbsMinus1.IsEmpty)
            {
                context.Destination[0].pHeightInSbsMinus1 = QuantumBinding.Utils.MarshalingUtils.MarshalBlittableArrayToPointer<ushort, Adamantium.Vulkan.Core.Interop.StdVideoAV1TileInfo>(stdVideoAV1TileInfo.PHeightInSbsMinus1.Span, ref context);
            }

        }
    }
}



