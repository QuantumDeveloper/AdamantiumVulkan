// ----------------------------------------------------------------------------------------------
// <auto-generated>
// This file was autogenerated by QuantumBindingGenerator.
// Do not edit this file manually because you will lose all your changes after the next generation.
// </auto-generated>
// ----------------------------------------------------------------------------------------------

using System;
using System.Runtime.InteropServices;
using QuantumBinding.Utils;
using Adamantium.Vulkan.Core.Interop;

namespace Adamantium.Vulkan.Core;

public unsafe partial class PhysicalDeviceSubgroupProperties : IMarshallableObject, IMarshallableFromPointer, IMarshallable<Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties>
{
    public PhysicalDeviceSubgroupProperties()
    {
    }

    public PhysicalDeviceSubgroupProperties(in Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties native)
    {
        MarshalFrom(in native);
    }

    public StructureType SType => StructureType.PhysicalDeviceSubgroupProperties;
    public object PNext { get; set; }
    public uint SubgroupSize { get; set; }
    public ShaderStageFlagBits SupportedStages { get; set; }
    public SubgroupFeatureFlagBits SupportedOperations { get; set; }
    public VkBool32 QuadOperationsInAllStages { get; set; }


    public static implicit operator PhysicalDeviceSubgroupProperties(Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties p)
    {
        return new PhysicalDeviceSubgroupProperties(in p);
    }

    public int GetSize()
    {
        var size = QuantumBinding.Utils.SizeOfCache<Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties>.Size;
        if (PNext is IMarshallableObject marshallable)
        {
            size += marshallable.GetSize();
        }
        return size;
    }

    public void MarshalTo(ref MarshallingContext<Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties> context)
    {
        new VkPhysicalDeviceSubgroupPropertiesMarshaller(this, ref context);
    }

    public void MarshalFrom(in Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties native)
    {
        if (PNext is IMarshallableFromPointer chainedPNext)
        {
            chainedPNext.MarshalFromPointer(native.pNext);
        }
        else
        {
            PNext = (System.IntPtr)native.pNext;
        }
        SubgroupSize = native.subgroupSize;
        SupportedStages = native.supportedStages;
        SupportedOperations = native.supportedOperations;
        QuadOperationsInAllStages = native.quadOperationsInAllStages;

    }
    public void* GetNativePointer<TContext>(ref TContext context) where TContext : IMarshallingContext, allows ref struct
    {
        var nativeSpan = context.AllocateNative<Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties>(1);
        var dataCursor = context.GetDataCursor();
        var internalContext = new MarshallingContext<Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties>(nativeSpan, dataCursor);
        this.MarshalTo(ref internalContext);
        context.SetDataCursor(internalContext.DataCursor);
        return System.Runtime.CompilerServices.Unsafe.AsPointer(ref nativeSpan[0]);
    }

    public void MarshalFromPointer(void* native)
    {
        if (native == null) return;
        MarshalFrom(in *(Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties*)native);
    }
    private ref struct VkPhysicalDeviceSubgroupPropertiesMarshaller
    {
        public VkPhysicalDeviceSubgroupPropertiesMarshaller(Adamantium.Vulkan.Core.PhysicalDeviceSubgroupProperties physicalDeviceSubgroupProperties, ref QuantumBinding.Utils.MarshallingContext<Adamantium.Vulkan.Core.Interop.VkPhysicalDeviceSubgroupProperties> context)
        {
            context.Destination[0].sType = physicalDeviceSubgroupProperties.SType;

            if (physicalDeviceSubgroupProperties.PNext is IMarshallableObject marshallable)
            {
                context.Destination[0].pNext = marshallable.GetNativePointer(ref context);
            }
            else if (physicalDeviceSubgroupProperties.PNext is System.IntPtr ptr)
            {
                context.Destination[0].pNext = (void*)ptr;
            }
            else if (physicalDeviceSubgroupProperties.PNext is nuint nPtr)
            {
                context.Destination[0].pNext = (void*)nPtr;
            }

            context.Destination[0].subgroupSize = physicalDeviceSubgroupProperties.SubgroupSize;

            context.Destination[0].supportedStages = physicalDeviceSubgroupProperties.SupportedStages;

            context.Destination[0].supportedOperations = physicalDeviceSubgroupProperties.SupportedOperations;

            if (physicalDeviceSubgroupProperties.QuadOperationsInAllStages != (uint)default)
            {
                context.Destination[0].quadOperationsInAllStages = physicalDeviceSubgroupProperties.QuadOperationsInAllStages;
            }

        }
    }
}



