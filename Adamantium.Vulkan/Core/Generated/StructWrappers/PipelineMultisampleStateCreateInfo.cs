// ----------------------------------------------------------------------------------------------
// <auto-generated>
// This file was autogenerated by QuantumBindingGenerator.
// Do not edit this file manually because you will lose all your changes after the next generation.
// </auto-generated>
// ----------------------------------------------------------------------------------------------

using System;
using System.Runtime.InteropServices;
using QuantumBinding.Utils;
using Adamantium.Vulkan.Core.Interop;

namespace Adamantium.Vulkan.Core;

public unsafe partial class PipelineMultisampleStateCreateInfo : IMarshallableObject, IMarshallableFromPointer, IMarshallable<Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo>
{
    public PipelineMultisampleStateCreateInfo()
    {
    }

    public PipelineMultisampleStateCreateInfo(in Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo native)
    {
        MarshalFrom(in native);
    }

    public StructureType SType => StructureType.PipelineMultisampleStateCreateInfo;
    public object PNext { get; set; }
    public VkPipelineMultisampleStateCreateFlags Flags { get; set; }
    public SampleCountFlagBits RasterizationSamples { get; set; }
    public VkBool32 SampleShadingEnable { get; set; }
    public float MinSampleShading { get; set; }
    public System.ReadOnlyMemory<VkSampleMask> PSampleMask { get; set; }
    public VkBool32 AlphaToCoverageEnable { get; set; }
    public VkBool32 AlphaToOneEnable { get; set; }


    public static implicit operator PipelineMultisampleStateCreateInfo(Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo p)
    {
        return new PipelineMultisampleStateCreateInfo(in p);
    }

    public int GetSize()
    {
        var size = QuantumBinding.Utils.SizeOfCache<Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo>.Size;
        if (PNext is IMarshallableObject marshallable)
        {
            size += marshallable.GetSize();
        }
        return size;
    }

    public void MarshalTo(ref MarshallingContext<Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo> context)
    {
        new VkPipelineMultisampleStateCreateInfoMarshaller(this, ref context);
    }

    public void MarshalFrom(in Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo native)
    {
        if (PNext is IMarshallableFromPointer chainedPNext)
        {
            chainedPNext.MarshalFromPointer(native.pNext);
        }
        else
        {
            PNext = (System.IntPtr)native.pNext;
        }
        Flags = native.flags;
        RasterizationSamples = native.rasterizationSamples;
        SampleShadingEnable = native.sampleShadingEnable;
        MinSampleShading = native.minSampleShading;
        var arrayLengthPSampleMask = ((uint)native.rasterizationSamples + 31) / 32;
        var tmpPSampleMask = new VkSampleMask[arrayLengthPSampleMask];
        QuantumBinding.Utils.MarshalingUtils.MarshalFromPointerToArray(native.pSampleMask, arrayLengthPSampleMask, tmpPSampleMask);
        PSampleMask = tmpPSampleMask;
        AlphaToCoverageEnable = native.alphaToCoverageEnable;
        AlphaToOneEnable = native.alphaToOneEnable;

    }
    public void* GetNativePointer<TContext>(ref TContext context) where TContext : IMarshallingContext, allows ref struct
    {
        var nativeSpan = context.AllocateNative<Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo>(1);
        var dataCursor = context.GetDataCursor();
        var internalContext = new MarshallingContext<Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo>(nativeSpan, dataCursor);
        this.MarshalTo(ref internalContext);
        context.SetDataCursor(internalContext.DataCursor);
        return System.Runtime.CompilerServices.Unsafe.AsPointer(ref nativeSpan[0]);
    }

    public void MarshalFromPointer(void* native)
    {
        if (native == null) return;
        MarshalFrom(in *(Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo*)native);
    }
    private ref struct VkPipelineMultisampleStateCreateInfoMarshaller
    {
        public VkPipelineMultisampleStateCreateInfoMarshaller(Adamantium.Vulkan.Core.PipelineMultisampleStateCreateInfo pipelineMultisampleStateCreateInfo, ref QuantumBinding.Utils.MarshallingContext<Adamantium.Vulkan.Core.Interop.VkPipelineMultisampleStateCreateInfo> context)
        {
            context.Destination[0].sType = pipelineMultisampleStateCreateInfo.SType;

            if (pipelineMultisampleStateCreateInfo.PNext is IMarshallableObject marshallable)
            {
                context.Destination[0].pNext = marshallable.GetNativePointer(ref context);
            }
            else if (pipelineMultisampleStateCreateInfo.PNext is System.IntPtr ptr)
            {
                context.Destination[0].pNext = (void*)ptr;
            }
            else if (pipelineMultisampleStateCreateInfo.PNext is nuint nPtr)
            {
                context.Destination[0].pNext = (void*)nPtr;
            }

            if (pipelineMultisampleStateCreateInfo.Flags != (uint)default)
            {
                context.Destination[0].flags = pipelineMultisampleStateCreateInfo.Flags;
            }

            context.Destination[0].rasterizationSamples = pipelineMultisampleStateCreateInfo.RasterizationSamples;

            if (pipelineMultisampleStateCreateInfo.SampleShadingEnable != (uint)default)
            {
                context.Destination[0].sampleShadingEnable = pipelineMultisampleStateCreateInfo.SampleShadingEnable;
            }

            context.Destination[0].minSampleShading = pipelineMultisampleStateCreateInfo.MinSampleShading;

            if (!pipelineMultisampleStateCreateInfo.PSampleMask.IsEmpty)
            {
                System.ReadOnlySpan<Adamantium.Vulkan.Core.Interop.VkSampleMask> sourceSpan = pipelineMultisampleStateCreateInfo.PSampleMask.Span;
                var byteSpan = context.AllocateData(sourceSpan.Length * sizeof(VkSampleMask));
                var destinationSpan = System.Runtime.InteropServices.MemoryMarshal.Cast<byte, VkSampleMask>(byteSpan);
                for (int i = 0; i < sourceSpan.Length; i++)
                {
                    destinationSpan[i] = sourceSpan[i];
                }
                var pDestination = (VkSampleMask*)System.Runtime.CompilerServices.Unsafe.AsPointer(ref System.Runtime.InteropServices.MemoryMarshal.GetReference(destinationSpan));
                context.Destination[0].pSampleMask = pDestination;
            }

            if (pipelineMultisampleStateCreateInfo.AlphaToCoverageEnable != (uint)default)
            {
                context.Destination[0].alphaToCoverageEnable = pipelineMultisampleStateCreateInfo.AlphaToCoverageEnable;
            }

            if (pipelineMultisampleStateCreateInfo.AlphaToOneEnable != (uint)default)
            {
                context.Destination[0].alphaToOneEnable = pipelineMultisampleStateCreateInfo.AlphaToOneEnable;
            }

        }
    }
}



