// ----------------------------------------------------------------------------------------------
// <auto-generated>
// This file was autogenerated by QuantumBindingGenerator.
// Do not edit this file manually, because you will lose all your changes after next generation.
// </auto-generated>
// ----------------------------------------------------------------------------------------------

using System.Runtime.InteropServices;
using QuantumBinding.Utils;
using AdamantiumVulkan.Core.Interop;

namespace AdamantiumVulkan.Core;

public unsafe partial class SetLatencyMarkerInfoNV : QBDisposableObject
{
    public SetLatencyMarkerInfoNV()
    {
    }

    public SetLatencyMarkerInfoNV(AdamantiumVulkan.Core.Interop.VkSetLatencyMarkerInfoNV _internal)
    {
        SType = _internal.sType;
        PNext = _internal.pNext;
        PresentID = _internal.presentID;
        Marker = _internal.marker;
    }

    public StructureType SType { get; set; }
    public void* PNext { get; set; }
    public ulong PresentID { get; set; }
    public LatencyMarkerNV Marker { get; set; }

    public AdamantiumVulkan.Core.Interop.VkSetLatencyMarkerInfoNV ToNative()
    {
        var _internal = new AdamantiumVulkan.Core.Interop.VkSetLatencyMarkerInfoNV();
        _internal.sType = SType;
        _internal.pNext = PNext;
        _internal.presentID = PresentID;
        _internal.marker = Marker;
        return _internal;
    }

    public static implicit operator SetLatencyMarkerInfoNV(AdamantiumVulkan.Core.Interop.VkSetLatencyMarkerInfoNV s)
    {
        return new SetLatencyMarkerInfoNV(s);
    }

}



