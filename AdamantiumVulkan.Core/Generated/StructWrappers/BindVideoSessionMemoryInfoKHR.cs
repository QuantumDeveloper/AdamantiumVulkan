// ----------------------------------------------------------------------------------------------
// <auto-generated>
// This file was autogenerated by QuantumBindingGenerator.
// Do not edit this file manually, because you will lose all your changes after next generation.
// </auto-generated>
// ----------------------------------------------------------------------------------------------

using System.Runtime.InteropServices;
using QuantumBinding.Utils;
using AdamantiumVulkan.Core.Interop;

namespace AdamantiumVulkan.Core;

public unsafe partial class BindVideoSessionMemoryInfoKHR : QBDisposableObject
{
    public BindVideoSessionMemoryInfoKHR()
    {
    }

    public BindVideoSessionMemoryInfoKHR(AdamantiumVulkan.Core.Interop.VkBindVideoSessionMemoryInfoKHR _internal)
    {
        SType = _internal.sType;
        PNext = _internal.pNext;
        MemoryBindIndex = _internal.memoryBindIndex;
        Memory = new DeviceMemory(_internal.memory);
        MemoryOffset = _internal.memoryOffset;
        MemorySize = _internal.memorySize;
    }

    public StructureType SType { get; set; }
    public void* PNext { get; set; }
    public uint MemoryBindIndex { get; set; }
    public DeviceMemory Memory { get; set; }
    public VkDeviceSize MemoryOffset { get; set; }
    public VkDeviceSize MemorySize { get; set; }

    public AdamantiumVulkan.Core.Interop.VkBindVideoSessionMemoryInfoKHR ToNative()
    {
        var _internal = new AdamantiumVulkan.Core.Interop.VkBindVideoSessionMemoryInfoKHR();
        _internal.sType = SType;
        _internal.pNext = PNext;
        _internal.memoryBindIndex = MemoryBindIndex;
        _internal.memory = Memory;
        _internal.memoryOffset = MemoryOffset;
        _internal.memorySize = MemorySize;
        return _internal;
    }

    public static implicit operator BindVideoSessionMemoryInfoKHR(AdamantiumVulkan.Core.Interop.VkBindVideoSessionMemoryInfoKHR b)
    {
        return new BindVideoSessionMemoryInfoKHR(b);
    }

}



