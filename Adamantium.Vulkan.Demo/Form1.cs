...
using ImageLayout = Adamantium.Vulkan.Core.ImageLayout;
using Result = Adamantium.Vulkan.Core.Result;

namespace Adamantium.Vulkan.Demo;
...
public unsafe partial class Form1 : Form
{
    ...
    { 
        ...
        Core.Constants.VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME,
    };
    ...
    private void Cleanup()
    {
        ...
        logicalDevice.DestroyImageView(depthStencilImageView);
    ...
    private void Render()
    {
        ...
        renderCommandBuffers[0] = commandBuffers[currentFrame];
    ...
    private void CreateInstance()
    {
        ...
        appInfo.ApplicationVersion = Core.Constants.VK_MAKE_API_VERSION(0, 1, 0, 0);
        appInfo.PEngineName = "Adamantium Engine";
        appInfo.EngineVersion = Core.Constants.VK_MAKE_API_VERSION(0, 1, 0, 0);
        appInfo.ApiVersion = Core.Constants.VK_API_VERSION_1_3;
    ...
    private void CreateLogicalDevice()
    {
        ...
        foreach (var queueFamily in uniqueQueueFamilies)
        {
            ...
            queueCreateInfo.PQueuePriorities = new[] {queuePriority};
    ...
    private void CreateBuffer(ulong size, BufferUsageFlagBits usage, MemoryPropertyFlagBits memoryProperties, out Buffer buffer, out DeviceMemory bufferMemory)
    {
        ...
        for (int i = 0; i < memProperties.MemoryTypeCount; i++)
        {
            if (((memoryRequirements.MemoryTypeBits >> (int)i) & 1) == 1
                && memProperties.MemoryTypes.Span[i].PropertyFlags.HasFlag(memoryProperties))
            {
    ...
    uint FindMemoryType(uint typeFilter, MemoryPropertyFlagBits properties)
    {
        ...
        {
            var shift = 1 << i;
            if ((typeFilter & shift) > 0 && memProperties.MemoryTypes.Span[i].PropertyFlags == properties)
            {
                return (uint)i;
            }
        }
    ...
    private void CreateCommandBuffers()
    {
        ...
        renderPassInfo.ClearValueCount = 1;
        renderPassInfo.PClearValues = new[] { clearValue };
    ...
    private Vertex[] GetVertexArray()
    {
        ...
        Vertex[] v =
        [
            // Front (+Z) - blue       (TexCoord V is Vulkan-convention: V=0 = top of image)
